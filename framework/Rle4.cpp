		...
		for (int x=0;x<map4.sx;x++)
		{
			...
			for (int s=0;s<count;s++,p++)
			{
				...
				for (;y1<y2;y1++,pt++) if (y1<map4.sy)
				{
					...
					int color = ( tcol >> 8 ) & 3;
					int intensity = 8*(tcol & 31);//
					intensity =((tcol & 255)*(tcol & 255))/255+10;

					uchar4 rgba;

					rgba.x = f_max(f_min( 255,  float (color_tab[color][0] * intensity )),0);//( tcol     &31)*8;
					rgba.y = f_max(f_min( 255,  float (color_tab[color][1] * intensity )),0) ;;//((tcol>>5) &31)*8;
					rgba.z = f_max(f_min( 255,  float (color_tab[color][2] * intensity )),0) ;;//((tcol>>10)&31)*8;
					rgba.w = 0;
