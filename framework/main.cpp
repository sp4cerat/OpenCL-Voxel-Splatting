...
{		
    ...
    if( cl_pbos[0] != 0 ) {
	  ...
	  //clSetKernelArg(cl_kernel_postx, 0, sizeof(cl_mem), (void *) &(cl_pbos[1]));
	  //clSetKernelArg(cl_kernel_postx, 1, sizeof(cl_mem), (void *) &(cl_pbos[0]));
...
int initCL(int argc, const char** argv)
{
    ...
    // Memory Setup
	if( bGLinterop ) {
        cl_pbos[0] = clCreateFromGLBuffer(cxGPUContext, CL_MEM_READ_WRITE, pbo_source, &ciErrNum);
        cl_pbos[1] = clCreateFromGLBuffer(cxGPUContext, CL_MEM_READ_WRITE, pbo_dest, &ciErrNum);
	} else {
        cl_pbos[0] = clCreateBuffer(cxGPUContext, CL_MEM_READ_WRITE, 4 * image_width * image_height, NULL, &ciErrNum);
        cl_pbos[1] = clCreateBuffer(cxGPUContext, CL_MEM_READ_WRITE, 4 * image_width * image_height, NULL, &ciErrNum);
	}
    oclCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);

    // Program Setup
    size_t program_length=0;//strlen(cl_kernel_src);
    const char* source_path = shrFindFilePath(clSourcefile, argv[0]);
    char *source = oclLoadProgSource(source_path, "", &program_length);//cl_kernel_src;//
...
void processImage()
{
    ...
    glBindBuffer(GL_PIXEL_PACK_BUFFER_ARB, pbo_source);//source);
    glReadPixels(0, 0, image_width, image_height, GL_BGRA, GL_UNSIGNED_BYTE, NULL); 

    //glClearColor(1,1,1,0);
    //glClear(GL_COLOR_BUFFER_BIT);
    glBindBuffer(GL_PIXEL_PACK_BUFFER_ARB, pbo_dest);//source);
    glReadPixels(0, 0, image_width, image_height, GL_BGRA, GL_UNSIGNED_BYTE, NULL); 
